...
export default function SignUpPage({ searchParams }: { searchParams: { phoneNumber?: string, countryCode?: string } }) {
  ...
  const countryCode = searchParams.countryCode;
      ...
      <div className="flex flex-col items-center justify-center text-center">
        ...
        <p className="max-w-[300px] md:max-w-[450px] text-center text-gray-700 text-[16px] md:text-[18px] lg:text-[20px]">Follow the link in the email we sent to ({countryCode}){phoneNumber}. The text message can take up to 1 minute to arrive.
        ...
        <div className="flex flex-row items-center justify-center">
          <button className="mb-[100px] md:mb-[0px] bg-white hover:bg-gray-100 text-black font-bold text-[18px] md:text-[25px] h-[40px] md:h-[60px] w-[270px] md:w-[350px] lg:w-[450px] rounded-[20px] border-1 border-gray-300" onClick={async () => {
            if (countryCode && phoneNumber) {
              sendAppLink(countryCode, phoneNumber, 'sms');
            }
